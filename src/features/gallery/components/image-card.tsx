...
export const ImageCard = ({
  ...
  const { toggleTagOnImage, userTags, appliedTags } = useTagStore();

  const favoriteTag = systemTags.find(
    (t) => t.tagName.toLowerCase() === "favorite",
  );

  const currentAppliedTags = appliedTags[image.id] || [];
  const isFavorited = favoriteTag
    ? currentAppliedTags.includes(favoriteTag.id)
    : false;
  ...
  return (
    ...
    >
      <div className="relative aspect-square rounded-3xl overflow-hidden bg-zinc-900 border border-white/5 shadow-sm group-hover:shadow-[0_20px_40px_rgba(0,0,0,0.5)] group-hover:border-white/10 transition-all duration-700">
        ...
        {/* Tags indicator */}
        <div className="absolute top-2 left-2 flex flex-wrap gap-1 p-1">
          {currentAppliedTags.map((tagId) => {
            const tag = [...userTags, ...systemTags].find(
              (t) => t.id === tagId,
            );
            if (!tag || tag.tagName.toLowerCase() === "favorite") return null;
            return (
              <div
                key={tagId}
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-x-2 group-hover:translate-x-0">
          ...
          >
            <Heart
              className={cn("w-4.5 h-4.5", isFavorited && "fill-current")}
            />
          </Button>
        </div>
